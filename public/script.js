document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    const FORM_SUBMITTING_CLASS = 'form--submitting';

    function getErrorContainer(field) {
      const id = field.id || field.name;
      return (
        contactForm.querySelector('[data-error-for="' + id + '"]') ||
    ...
    function validateField(field) {
      ...
      if (required && value === '') {
        showError(field, label.trim().replace(/:$/, '') + ' is required.');
        return false;
      }
      ...
      if (value !== '') {
        ...
        const minLength = parseInt(field.dataset.minLength || field.minLength, 10);
        if (!isNaN(minLength) && minLength > 0 && value.length < minLength) {
          showError(field, label.trim().replace(/:$/, '') + ' must be at least ' + minLength + ' characters.');
          return false;
        }

        const maxLength = parseInt(field.dataset.maxLength || field.maxLength, 10);
        if (!isNaN(maxLength) && maxLength > 0 && value.length > maxLength) {
          showError(field, label.trim().replace(/:$/, '') + ' must be no more than ' + maxLength + ' characters.');
          return false;
        }
      }
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      if (contactForm.classList.contains(FORM_SUBMITTING_CLASS)) return;

      const submitBtn = contactForm.querySelector('[type="submit"]');
      const originalBtnText = submitBtn ? submitBtn.textContent : '';

      contactForm.classList.add(FORM_SUBMITTING_CLASS);
      if (submitBtn) {
      ...
      fetch(action, {
        ...
        .then(function () {
          contactForm.classList.remove(FORM_SUBMITTING_CLASS);
          contactForm.classList.add(FORM_SUCCESS_CLASS);
          contactForm.reset();

          const successMsg = contactForm.querySelector('.form-success-message, [data-success-message]');
          if (successMsg) {
        ...
        .catch(function () {
          ...
          const errorMsg = contactForm.querySelector('.form-error-message, [data-form-error]');
          if (errorMsg) {
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'fad74927-0787-4767-868d-82f8451d2e6b',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'fad74927-0787-4767-868d-82f8451d2e6b';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'fad74927-0787-4767-868d-82f8451d2e6b';
